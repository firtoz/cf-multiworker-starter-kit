...
Real keys: **`.env.local`** (dev), **`.env.staging`** (staging + PR preview deploys), **`.env.production`** (prod). **`.env.example`** is documentation only. Never commit secrets.

**When adding a key:** follow the full checklist in [workers-env-local/SKILL.md](../workers-env-local/SKILL.md) (includes **`turbo.json` `globalEnv`**, deploy workflow `env:` maps, `env.requirements.ts`, Alchemy bindings). Run **`bun run check:ci-env-wiring`**. Always-apply: [new-env-var-checklist.mdc](../../rules/new-env-var-checklist.mdc). Alchemy Secret docs: [Alchemy Secret](https://alchemy.run/providers/cloudflare/secret/).
