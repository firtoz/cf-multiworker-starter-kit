...
- Root scripts use Turbo; web dev/deploy/destroy uses **`alchemy-cli.ts`** with **`frontend`** (see **`ALCHEMY_APP_IDS`** / **`PRODUCT_PREFIX`** in root README — **Name your product** → **Code-first infra names**).
