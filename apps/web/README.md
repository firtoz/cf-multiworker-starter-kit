...
- Public auth URL is computed at deploy/dev time (no dotfile key) — see [auth-setup](../../.agents/skills/auth-setup/SKILL.md).
...
- Fork setup: [`.agents/skills/auth-setup/SKILL.md`](../../.agents/skills/auth-setup/SKILL.md).
...
- [.agents/skills/durable-object-package/SKILL.md](../../.agents/skills/durable-object-package/SKILL.md) — package layout and `alchemy.run.ts`
- [.agents/skills/web-alchemy-bindings/SKILL.md](../../.agents/skills/web-alchemy-bindings/SKILL.md) — `apps/web/package.json` workspace dep, `alchemy.run.ts` bindings, `bun run typegen`
...
**Development:** Run root **`bun run setup`** / **`setup:local`** once (interactive **variable browser** in a TTY, or **`-- --yes`** / **`CI=true`** for auto-generated Alchemy + chatroom secrets only), or add values to repo-root **`.env.local`** (or optional per-package **`.env.local`**), not a plain **`.env`** — see [.agents/skills/workers-env-local/SKILL.md](../../.agents/skills/workers-env-local/SKILL.md) and root **[AGENTS.md](../../AGENTS.md)** (index):
```bash
MY_SECRET=dev-value
```
