...
export function OAuthStagingProxyNotice({ className }: OAuthStagingProxyNoticeProps) {
	return (
		...
		>
			Google and GitHub on this preview (sign-in, sign-up, guest upgrade, connect on account)
			route through the staging OAuth callback, then return here. Email/password auth stays on
			this preview. The staging stack must be deployed.
		</p>
	);
}
