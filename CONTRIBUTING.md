...
- **`bun run github:sync:staging`** / **`github:sync:prod`** — Local/admin-only **`stacks/admin.ts`** run: upserts GitHub Environment **secrets** (`ALCHEMY_PASSWORD`, `ALCHEMY_STATE_TOKEN`, `CHATROOM_INTERNAL_SECRET`, `CLOUDFLARE_API_TOKEN`) and **variables** (`CLOUDFLARE_ACCOUNT_ID`, `CF_STARTER_DEPLOY_ENABLED`; deploy flag defaults to `true` when syncing). Optionally merges **`GITHUB_ENV_*`** deployment protection (reviewers, wait timer, branch policy) when set — see **`stacks/github-repository-environment-from-env.ts`**. Uses **`gh auth token`** / **`gh repo view`** by default. Do not run from normal CI/deploy.
- **`bun run github:env:staging`** / **`github:env:prod`** — **`stacks/github-environment-only.ts`**: GitHub Environment + deployment protection only (no secrets, no `.env.*`). Same **`GITHUB_ENV_*`** knobs; use when you only need workflow deployment gates.
