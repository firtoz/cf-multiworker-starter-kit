...
- Alchemy infra naming: literals in **`alchemy.run.ts`** (**`await alchemy`**) and **[`packages/alchemy-utils/worker-peer-scripts.ts`](packages/alchemy-utils/worker-peer-scripts.ts)** — no env-var branding. README explains **`--filter`** vs **`--app`**.
