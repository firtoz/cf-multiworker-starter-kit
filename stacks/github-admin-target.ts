...
/** Login for the current **`gh`** session (or **`GITHUB_ACTOR`** if set — e.g. CI). */
export function getGitHubActorLogin(): string {
	return (
		optionalEnv("GITHUB_ACTOR") ??
		runGh(
			["api", "user", "--jq", ".login"],
			"Could not read your GitHub username. Run `gh auth login`, set GITHUB_ACTOR, or set **`github.environments.stagingFork.reviewerUsers`** in **`config/github.policy.ts`**.",
		)
	);
}
