...
3. **`ALCHEMY_PASSWORD`**, **`ALCHEMY_STATE_TOKEN`**, and CI — Three different concepts; **`ALCHEMY_PASSWORD`** ≠ **`ALCHEMY_STATE_TOKEN`**.
   ...
   - **`ALCHEMY_STATE_TOKEN`** — authenticates HTTPS access to the [Cloudflare-backed Alchemy state store](https://alchemy.run/guides/cloudflare-state-store/) Worker (**`alchemy-state-service`** on that account). [Alchemy’s guide](https://alchemy.run/guides/cloudflare-state-store/) expects **the same token for all deployments on the same Cloudflare account**. In this repo, non-local deploys (**`staging`**, **`pr-<n>`**, **`prod`**) targeting **one account** must all use **the identical** `ALCHEMY_STATE_TOKEN` literal in dotfiles **and** in GitHub Environment **secrets** (otherwise you get **`[CloudflareStateStore] The token is invalid`** / **401** when a stage uses a mismatched token). **`STAGE=local`** **`alchemy dev`** uses the filesystem **`.alchemy/`** store and does **not** use this token for CloudflareStateStore—only **`deploy`** / **`destroy`** with non-local **`STAGE`** need it locally.
     - After changing **`ALCHEMY_STATE_TOKEN`** in **`.env.staging`** / **`.env.production`**, run **`bun run github:sync:staging`** / **`github:sync:prod`** (from a trusted machine) so **GitHub Actions** picks up the same value. CI deploys only see what GitHub exposes—dotfiles alone are not enough.
     - Different **Cloudflare accounts** for staging vs production would imply **different** state stores and potentially different **`ALCHEMY_STATE_TOKEN`** values; the usual starter setup assumes **one account** and thus **one** state token.
