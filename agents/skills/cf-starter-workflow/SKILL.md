...
- **Repo policy (not dotenv)** — Merge toggles, rulesets, and Environment deployment rules live in **[`config/github.policy.ts`](../../../config/github.policy.ts)** under **`github.sync.*`** and **`github.environments.*`**. Staging sync can apply REST + rulesets from that file; see README *GitHub admin sync reference*, [`github-repository-settings-sync.ts`](../../../stacks/github-repository-settings-sync.ts), [`github-repo-rulesets-sync.ts`](../../../stacks/github-repo-rulesets-sync.ts).
