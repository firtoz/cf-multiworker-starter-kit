...
export const authWorkerApp = new Hono<{ Bindings: CloudflareEnv; Variables: AppVariables }>();

authWorkerApp.use("*", async (c, next) => {
...
authWorkerApp.get("/health", (c) => c.json({ ok: true }));

registerAdminRoutes(authWorkerApp);
registerProfileRoutes(authWorkerApp);
registerAuthProvidersRoute(authWorkerApp);

authWorkerApp.on(["GET", "POST"], "/api/auth/*", async (c) => {
