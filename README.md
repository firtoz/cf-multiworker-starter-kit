...
**Repository rulesets (staging sync only):** when **`github.sync.applyRulesets`** is **true** in **`config/github.policy.ts`**, **`github:sync:staging`** upserts rulesets for **`main`** and **`production`**: pull-request requirements, no force-push, optional **required status checks** on **`main`** (see **`github.repository.rulesets.main.requiredStatusCheckContexts`**), and on **`production`** a **workflows** rule that gates merges using [`restrict-production-pr-source.yml`](.github/workflows/restrict-production-pr-source.yml). Repo variable **`CF_STARTER_PRODUCTION_PR_HEAD`** is set from **`github.repository.rulesets.production.sourceBranchForProductionPrs`**. See [`stacks/github-repo-rulesets-sync.ts`](stacks/github-repo-rulesets-sync.ts).
