...
3. **`.env.local`**: After **`bun run setup`**, **`ALCHEMY_PASSWORD`** and **`CHATROOM_INTERNAL_SECRET`** exist; optional **`CLOUDFLARE_API_TOKEN`** and **`CLOUDFLARE_ACCOUNT_ID`** if you do not use the profile; see [Alchemy’s Cloudflare auth guide](https://alchemy.run/guides/cloudflare/). The token and account id must belong to the **same** Cloudflare account (wrong pairing often shows up as **`workers.dev`** / Alchemy state-store errors). See [`.env.example`](.env.example) for all keys.
...
[`ci.yml`](.github/workflows/ci.yml) (**Quality checks**) runs **one verify job** on pushes and PRs to **`main`**: Drizzle generated-artifact guard, **`bun run typecheck`** (Turbo pulls `typegen` where needed), **`bun run lint`**, seeded **`.env.local`**, then **`bun run build`**. CI uses **`actions/checkout@v6`** and **`oven-sh/setup-bun`** (pinned **`bun-version`**); **[`.nvmrc`](.nvmrc)** remains for **local** editors and tooling that expect a repo Node pin — workflows do **not** run **`actions/setup-node`**. **Deploy staging** ([`deploy-staging.yml`](.github/workflows/deploy-staging.yml)) runs only after that workflow **succeeds** on a **`push` to `main`**, on the same commit. **`deploy-production.yml`** and **`deploy-pr-preview.yml`** are separate; workflows **no-op successfully** on fresh forks until **`CF_STARTER_DEPLOY_ENABLED=true`** is set on the GitHub Environment (see [Deploy from your machine](#deploy-from-your-machine)). After Turbo deploy, staging / prod / preview jobs read **`workers.dev`** from **`${GITHUB_WORKSPACE}/.alchemy/ci/cf-starter-web-deploy-url.txt`** (written by **`apps/web/alchemy.run.ts`** when **`GITHUB_ACTIONS`** is set; **`.alchemy/`** is gitignored) into **`GITHUB_OUTPUT`**, Actions **Summary**, and **`::notice`**.
