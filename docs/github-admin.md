...
- **PR previews** (`.github/workflows/pr-deploy.yml`): **same-repo** PRs run Quality in this workflow, then preview deploy to **`staging`** when **`DEPLOY_ENABLED`**; sticky PR comments and optional GitHub Deployments under the shared Environment **`preview`** (payload/`description` carry the PR number). **Fork PRs** run **Quality only** (no preview deploy, no **`staging-fork`** preview path). Preview stacks use **`STAGE=pr-<n>`**. **Teardown** on **`pull_request` `closed`** (same-repo only): **`verify:deploy-env:preview`**, then **`destroy:preview`**, and on destroy success inactivates that PR’s GitHub Deployments. Default **`GITHUB_TOKEN` cannot delete Environments** (GitHub returns **Resource not accessible by integration**); new previews therefore use the shared **`preview`** Environment instead of per-PR **`preview-pr-<n>`**. Optional repository secret **`PREVIEW_ENV_ADMIN_TOKEN`** (classic PAT with **`repo`**, or a GitHub App token with Environments write) enables deleting leftover legacy **`preview-pr-*`** Environments during teardown. Checks out the **base** branch so destroy does not run untrusted PR scripts. Alchemy state for all non-local stages lives in the shared account **`alchemy-state-service`** Worker — teardown does **not** delete it. Operators can sweep leftover **exact** Alchemy preview resources (see **`alchemy-utils/preview-pr-resources`** catalogs; keep **`bun run check:preview-catalog`** green when adding resources) and stale **`preview-pr-*`** Environments with **`bun run preview:cleanup:orphans`** (dry-run by default; add **`--apply`** to delete; open PRs are auto-excluded unless **`--include-open`**; incomplete CF/GitHub inventory aborts **`--apply`**; uses your **`gh`** credentials, which can delete Environments). **`alchemy destroy`** still loads **`alchemy.run.ts`** top-level **`requireEnv`** — mirror deploy **`env`** keys on **Destroy PR preview** when you add vars at module scope (see [**multiworker-gotchas** §22](../.agents/skills/multiworker-gotchas/SKILL.md)).
