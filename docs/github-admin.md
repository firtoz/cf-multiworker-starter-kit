...
- **Branch protection:** the **`main`** ruleset requires **`Quality reusable / Quality checks`** (aggregator job at the end of [`.github/workflows/quality-reusable.yml`](../.github/workflows/quality-reusable.yml)) so the same context applies to **push** (`Main`) and **pull_request** (`PR preview`) callers. Default list: **`DEFAULT_GITHUB_POLICY.github.repository.rulesets.main.requiredStatusCheckContexts`** in [`github-policy-config.ts`](../packages/alchemy-utils/src/github-policy-config.ts). If sync blocks merges, compare strings to the **Checks** tab / API, then edit that default or override in [`config/github.policy.ts`](../config/github.policy.ts).
