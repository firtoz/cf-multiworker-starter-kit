...
- **Main** (`.github/workflows/main-push.yml`): **push** **`main`** → Quality → **staging** deploy (`maybe_production_pr` when **`MULTIWORKER_AUTO_PRODUCTION_PR`**). Uses Environment **`staging`**.
...
- **Branch protection:** the **`main`** ruleset requires the **Quality gate** check by context string. GitHub names reusable-workflow jobs as **`{workflow name} / {caller job id} / {inner job name}`** (e.g. **`PR preview / Quality / Quality checks`** on PRs, **`Main / Quality / Quality checks`** on push to `main`) — not **`Quality reusable / …`**. Default: **`PR preview / Quality / Quality checks`** in [`github-policy-config.ts`](../packages/alchemy-utils/src/github-policy-config.ts). If it stays **Expected — Waiting for status**, the names are wrong; copy the exact string from the PR **Checks** tab after **`bun run github:sync:staging`**. Listing both PR and push names would usually **block** PRs (push-only contexts never run on the PR commit).
