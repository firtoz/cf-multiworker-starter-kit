...
**Related:** [Auth worker checklist](../.agents/skills/auth-setup/SKILL.md) · [`.env.example`](../.env.example) (auth section) · [GitHub Environments sync](github-admin.md) · [Multi-environment (local / staging / prod)](#multi-environment-setup-local-staging-production)
...
Optional: set **`WEB_DOMAINS=app.example.com`** in `.env.production` (and staging if needed) **before** deploy so the URL is stable and known up front. See [workers-env-local](../.agents/skills/workers-env-local/SKILL.md).
...
Stock Better Auth `oAuthProxy` only hooks **`/sign-in/social`**; this starter kit extends it with a composed plugin for **`/link-social`** on PR passthrough (see [`oauth-proxy-passthrough-patch.ts`](../workers/auth-worker/src/oauth-proxy-passthrough-patch.ts) — `createPassthroughOAuthProxyPlugin`). **Redeploy staging** after changing that plugin — staging receives the OAuth callback before the preview finishes linking.
...
| **403** on sign-in (especially anonymous → OAuth) | Untrusted `Origin` | Add origin in `/admin/origins` or `AUTH_SEED_ORIGINS`; see [auth-setup](../.agents/skills/auth-setup/SKILL.md) |
