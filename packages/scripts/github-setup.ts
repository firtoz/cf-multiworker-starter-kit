	...
	if (stagingOnly) {
		note(
			[
				...
				"",
				"Optional **without** `.env.staging`: `bun run github:env:staging` — Alchemy **`RepositoryEnvironment`** only (deployment reviewers / wait timer / branch policy via `GITHUB_ENV_*` vars). Does **not** sync secrets.",
				"",
	...
	if (prodOnly) {
		note(
			[
				...
				"",
				"Optional **without** `.env.production`: `bun run github:env:prod` — environment shell + deployment protection only (`GITHUB_ENV_*`). Does **not** sync secrets.",
				"",
	...
	if (scope === "doc") {
		note(
			[
				...
				"**Environment only (no dotfile):** `bun run github:env:staging` / `github:env:prod` — see root `package.json` and `stacks/github-environment-only.ts`.",
				"",
